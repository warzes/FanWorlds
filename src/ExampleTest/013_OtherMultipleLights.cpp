...
{
	Print("_013OtherMultipleLights Create");
...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    m_cube = renderSystem.CreateGeometryBuffer(BufferUsage::StaticDraw, Countof(vertices)/8, sizeof(float)*8, vertices, m_lightingShader);
...
void _013OtherMultipleLights::Destroy()
{
	...
	Print("_013OtherMultipleLights Destroy");
}
//-----------------------------------------------------------------------------
